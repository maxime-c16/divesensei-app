...
OUT_DATASET_JSON = ROOT / "outputs/r17_approve_benchmark_dataset.json"
OUT_DATASET_MD = ROOT / "outputs/r17_approve_benchmark_dataset.md"
OUT_BENCH_JSON = ROOT / "outputs/r17_high_precision_approve_coverage_benchmark.json"
OUT_BENCH_MD = ROOT / "outputs/r17_high_precision_approve_coverage_benchmark.md"
OUT_CMP_JSON = ROOT / "outputs/r17_approve_candidate_comparison.json"
OUT_CMP_MD = ROOT / "outputs/r17_approve_candidate_comparison.md"
OUT_BEST_JSON = ROOT / "outputs/r17_best_approve_policy.json"
OUT_BEST_MD = ROOT / "outputs/r17_best_approve_policy.md"
...
def candidate_row(name: str, desc: str, internal: dict[str, Any], external: dict[str, Any], policy: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "approve_review_mode_interesting": interesting and external["approve_coverage"] > 0.0909,
        "internal": internal,
        "external": external,
    }
...
def main() -> None:
    ...
    viable = (
        ...
        and best["external_approve_coverage"] >= 0.15
    )
    decision = "R17_APPROVE_REVIEW_MODE_VIABLE" if viable else "R17_APPROVE_REVIEW_MODE_NOT_YET_VIABLE"

    benchmark_rows = [row for row in internal_rows + external_rows if row["role"] != "other" or row["r9_score"] >= 0.50]
    dataset = {
        "dataset_name": "r17_approve_benchmark_dataset",
    ...
    report = {
        "experiment_name": "r17_high_precision_approve_coverage_benchmark",
        ...
        "stopping_rule_result": "safe_approve_coverage_above_0_15_found" if viable else "no_candidate_crossed_0_15_with_zero_dangerous_approves",
    }
    comparison_payload = {
        "experiment_name": "r17_approve_candidate_comparison",
        "final_decision": decision,
        "best_candidate": best["candidate"],
        "candidate_count": len(comparison),
        "safe_candidate_count": len(interesting),
    ...
    best_payload = {
        "experiment_name": "r17_best_approve_policy",
        "final_decision": decision,
        "best_candidate": best["candidate"],
        "best_candidate_metrics": best,
        "product_interpretation": "Approve/review mode is viable at the bounded benchmark bar." if viable else "Approve/review mode is not yet viable; safe coverage remains below 0.15.",
    }
    ...
    md = "\n".join([
        "# r17 High-Precision Approve Coverage Benchmark",
        "",
    ...
    OUT_CMP_MD.write_text(md.replace("# r17 High-Precision Approve Coverage Benchmark", "# r17 Approve Candidate Comparison"), encoding="utf-8")
    OUT_BEST_MD.write_text(
        "\n".join([
            "# r17 Best Approve Policy",
            "",
            f"- final decision: `{decision}`",
            f"- best candidate: `{best['candidate']}`",
