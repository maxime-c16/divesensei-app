...
PROBE3_PATH = Path("outputs/platform_noise_feature_probe_r3.json")
OUT_JSON = Path("outputs/platform_noise_feature_probe_r4.json")
OUT_MD = Path("outputs/platform_noise_feature_probe_r4.md")
SAMPLE_RATE = 16_000
...
def spectral_contrast_features(power_post: np.ndarray, freqs: np.ndarray) -> tuple[float, float]:
    ...
    contrast_low_high_slope = hi_mean - lo_mean
    return contrast_mean_post, contrast_low_high_slope
...
def onset_flux_envelope(windowed: np.ndarray) -> np.ndarray:
    ...
    flux = flux / max(float(np.max(flux)), eps)
...
def tempogram_peak_ratio(onset_env: np.ndarray, min_lag: int, max_lag: int) -> float:
    ...
    for lag in range(0, max_lag + 1):
        ...
        b = onset_env[lag:]
        lags.append(float(np.dot(a, b)))
    ac = np.asarray(lags, dtype=np.float64)
    non_zero_peak = float(np.max(ac[min_lag : max_lag + 1])) if max_lag >= min_lag else 0.0
    return non_zero_peak / max(float(ac[0]), eps)
...
def extract_features(signal: np.ndarray, sample_rate: int) -> dict[str, float]:
    ...
    whistle_band_energy_fraction_post = float(
        np.mean(np.sum(p[:, whistle_band], axis=1) / np.maximum(np.sum(p, axis=1), eps))
    )
    ...
    # 24 ms to 200 ms lag region at 8 ms frame step (hop=128 @16k)
...
def write_md(path: Path, report: dict) -> None:
    comp = report["five_way_comparison"]
    lines = [
        "# Platform/Noise Feature Probe (r4 final bundle)",
        "",
    ...
    for feature in report["part_a_feature_bundle"]["features"]:
        lines.append(f"- `{feature['name']}`: {feature['definition']} ({feature['reference_idea']})")
    lines.extend(
        [
            f"- offline audio/window derived: `{report['part_a_feature_bundle']['derived_from_offline_audio_windows']}`",
            "",
            "## Part B — Five-way comparison",
            "",
    ...
    for key, label in [
        ("baseline_current_feature_set", "baseline current feature set"),
        ("probe_r1", "platform/noise probe r1"),
        ("probe_r2", "platform/noise probe r2"),
        ("probe_r3", "platform/noise probe r3"),
        ("probe_r4_final_bundle", "platform/noise probe r4 (final bundle)"),
    ]:
        item = comp[key]
        lines.append(
            f"| {label} | {item['auc']:.4f} | {item['macro_f1']:.4f} | {item['accuracy']:.4f} | {item['platform_dive_recall']:.4f} | {item['noise_or_other_recall']:.4f} | `{item['confusion_matrix']}` |"
        )
    ...
    lines.extend(
        [
            ...
            f"- r4 improved over accepted r2: `{report['error_comparison']['r4_improves_over_r2']}`",
...
def main() -> None:
    ...
    probe3 = json.loads(PROBE3_PATH.read_text())
    ...
    r4 = evaluate(train, holdout, fmap)
    ...
    r3 = probe3["four_way_comparison"]["probe_r3_with_tonal_noise_penalty"]
    ...
    fn_counts = [
        ...
        r4["confusion_matrix"][0][1],
    ]

    r4_better_than_r2 = (
        r4["auc"] > r2["auc"]
        and r4["confusion_matrix"][1][0] <= r2["confusion_matrix"][1][0]
        and r4["platform_dive_recall"] >= r2["platform_dive_recall"]
    )
    harmful = "none_detected"
    if r4["platform_dive_recall"] < r2["platform_dive_recall"]:
        harmful = "platform_recall_regression_vs_r2"
    elif r4["confusion_matrix"][1][0] > r2["confusion_matrix"][1][0]:
        harmful = "noise_to_platform_fp_regression_vs_r2"

    decision = (
        "PLATFORM_NOISE_R4_READY_FOR_PHASE5_R7"
        if r4_better_than_r2
        else "STOP_FEATURE_ITERATION_AND_DOCUMENT_NEAR_PASS"
    )
    rationale = (
        "r4 is clearly better than accepted r2 on residual ranking/error counts and is ready for Phase 5 r7 integration."
        if r4_better_than_r2
        else "r4 does not clearly beat accepted r2 on the residual error problem; stop feature iteration and document near-pass."
    )
    ...
    report = {
        ...
        "part_a_feature_bundle": {
            "bundle_id": "platform_noise_feature_bundle_r4_candidate",
            "added_feature_count": 4,
            "only_required_4_features_added": True,
            "features": [
                {
                    "name": "spectral_contrast_mean_post",
                    "definition": "Mean octave-band peak-vs-valley log contrast over post-impact frames.",
                    "reference_idea": "spectral contrast (peak-vs-valley structure)",
                    "offline_window_derived": True,
                },
                {
                    "name": "spectral_contrast_low_high_slope_post",
                    "definition": "High-band minus low-band spectral contrast mean over post-impact frames.",
                    "reference_idea": "broadband-vs-tonal band-structure slope",
                    "offline_window_derived": True,
                },
                {
                    "name": "onset_tempogram_peak_ratio_post",
                    "definition": "Max non-zero-lag local autocorrelation ratio of post-impact onset-flux envelope.",
                    "reference_idea": "tempogram/local autocorrelation rhythmic clutter cue",
                    "offline_window_derived": True,
                },
                {
                    "name": "onset_density_0_300ms_post",
                    "definition": "Count of local onset-flux peaks in first 300 ms post-impact.",
                    "reference_idea": "onset-density compactness vs diffuse clutter",
                    "offline_window_derived": True,
                },
            ],
            "derived_from_offline_audio_windows": True,
            "no_other_new_feature_added": True,
        },
        "five_way_comparison": {
