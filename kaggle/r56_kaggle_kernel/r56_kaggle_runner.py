...
SESSION_PACKAGE_SLUGS = ["divesensei-r56-burst-validation-sessions"]
...
def _find_session_package() -> Path:
    ...
    for slug in SESSION_PACKAGE_SLUGS:
        candidates.extend([Path("/kaggle/input") / slug, Path("/kaggle/input/datasets/maximecauchy") / slug])
    candidates.extend(Path("/kaggle/input").glob("**/r56_burst_validation_sessions"))
    for root in candidates:
        if (root / "outputs").exists():
            return root
        nested = root / "r56_burst_validation_sessions"
        if (nested / "outputs").exists():
            return nested
    raise FileNotFoundError("Could not find r56 burst validation session package")
...
def main() -> int:
    ...
    output_root = Path("/kaggle/working/r56_burst_validation")
    ...
    (output_root / "r56_remote_validation_health.json").write_text(json.dumps(health, indent=2))
    ...
    for fps in FPS_VALUES:
        ...
        for session in SESSIONS:
            ...
            session_output = output_root / session / f"false_controls_full_frame_{fps_label}fps"
            cmd = [
                sys.executable,
                "-m",
                "divesensei.cli",
        ...
        if any(run.get("returncode") != 0 for run in runs):
            break
    summary = {"benchmark_id": "r56_burst_validation_remote_scoring", "health": health, "runs": runs}
    (output_root / "r56_remote_validation_summary.json").write_text(json.dumps(summary, indent=2))
    _zip_outputs(output_root, Path("/kaggle/working/r56_burst_validation_bundle.zip"))
    if any(run.get("returncode") != 0 for run in runs):
        return 1
    return 0
