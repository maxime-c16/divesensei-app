...
SESSION_PACKAGE_SLUGS = ["divesensei-r54-visual-burst-sessions"]
...
def _find_session_package() -> Path:
    ...
    for slug in SESSION_PACKAGE_SLUGS:
        candidates.extend([Path("/kaggle/input") / slug, Path("/kaggle/input/datasets/maximecauchy") / slug])
    candidates.extend(Path("/kaggle/input").glob("**/r54_visual_burst_sessions"))
    for root in candidates:
        if (root / "outputs").exists():
            return root
        nested = root / "r54_visual_burst_sessions"
        if (nested / "outputs").exists():
            return nested
    raise FileNotFoundError("Could not find r54 visual burst session package")
...
def main() -> int:
    ...
    output_root = Path("/kaggle/working/r54_audio_local_visual_burst")
    ...
    health = {
        ...
        "window_seconds": [-3.0, 3.0],
    }
    (output_root / "r54_remote_burst_health.json").write_text(json.dumps(health, indent=2))
    ...
    for fps in FPS_VALUES:
        ...
        for session in SESSIONS:
            ...
            targets = json.loads((source / "r54_burst_targets.json").read_text()).get("targets", [])
            if not targets:
                runs.append({"session_id": session, "fps": fps, "returncode": 0, "skipped": "no_targets"})
                continue
            session_output = output_root / session / f"burst_full_frame_{fps_label}fps"
            cmd = [
                sys.executable,
                "-m",
                "divesensei.cli",
                ...
                "--fps",
                str(fps),
                "--audio-gate-pre-seconds",
                "3.0",
                "--audio-gate-post-seconds",
                "3.0",
                "--model-id",
                model_id,
                "--model-cache-dir",
    ...
    summary = {"benchmark_id": "r54_audio_local_visual_burst_remote_scoring", "health": health, "runs": runs}
    (output_root / "r54_remote_burst_summary.json").write_text(json.dumps(summary, indent=2))
    _zip_outputs(output_root, Path("/kaggle/working/r54_audio_local_visual_burst_bundle.zip"))
    if any(run.get("returncode") != 0 for run in runs):
        return 1
    return 0
