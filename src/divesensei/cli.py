...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if argv[0] == "export-event-window-manifest":
        if any(flag in argv[1:] for flag in ("-h", "--help")):
            from divesensei.workflows.event_window_manifest import build_parser as event_window_manifest_build_parser

            event_window_manifest_build_parser().print_help()
            return 0
    ...
    if argv[0] == "export-event-reviewed-manifest":
        if any(flag in argv[1:] for flag in ("-h", "--help")):
            from divesensei.workflows.event_reviewed_manifest import build_parser as reviewed_manifest_build_parser

            reviewed_manifest_build_parser().print_help()
            return 0

        from divesensei.workflows.event_reviewed_manifest import main as reviewed_manifest_main

        return reviewed_manifest_main(argv[1:])
